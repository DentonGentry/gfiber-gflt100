...
static void spu_ah_digest_done (struct spu_trans_req *trans_req)
{
    ...
    ptr = buf + sizeof(struct iphdr) + sizeof(struct ip_auth_hdr);
    ...
    memcpy(ptr, hmac, MAX_AH_AUTH_LEN);

    SPU_DATA_DUMP("spu_ah_digest_done-AH", buf, (req->nbytes + 32));
    ...
    if(((trans_req->sfrags > 1) || (trans_req->alloc_buff_spu)) && (trans_req->dbuf))
                                      ...
                                      int nbytes)
{
    ...
    buffer_len = nbytes + MAX_AH_AUTH_LEN + RX_MSG_HDR_SIZE 
                                          + RX_STS_SIZE 
                                          + ALIGN_SIZE;
    trans_req->dlen = ((buffer_len / ALIGN_SIZE) * ALIGN_SIZE);
