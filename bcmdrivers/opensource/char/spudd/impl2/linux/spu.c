                                    ...
                                    int len)
{
    ...
    tmp = buf;
    tmp += (RX_MSG_HDR_SIZE + sizeof(struct iphdr) + RX_ESP_HDR_SIZE + ivsize);
...
static int process_ipsec_esp_response(struct spu_trans_req *trans_req)
{
    ...
    if(((trans_req->dfrags > 1) || (trans_req->alloc_buff_spu)) && (trans_req->dbuf))
    {
...
struct spu_trans_req *spu_alloc_trans_req(uint32 cryptoflags)
{
    ...
    trans_req->sfrags = 0;
    trans_req->dfrags = 0;
    trans_req->sfrags_list = NULL;
    trans_req->dfrags_list = NULL;
    trans_req->alloc_buff_spu = 0;
                                 ...
                                 int ivsize)
{
    ...
    {
	    ptr = page_address (sg_page (sg)) + sg->offset;
	    SPU_DATA_DUMP("sc_list_to_dma_in_list", ptr, sg->length);
	    if(!buffer_len)
	    {
	        if ((trans_req->sfrags == 1) &&
                (trans_req->dir == BCMSAD_DIR_OUTBOUND))
	        {
		        buffer_len = cryptlen + ivsize + sizeof (struct iphdr) 
                                           + RX_ESP_HDR_SIZE;
	        }
	        else
	        {
		        buffer_len = sg->length + ivsize + sizeof (struct iphdr) 
                                           + RX_ESP_HDR_SIZE;
	        }
	        ptr = ptr - (ivsize + sizeof (struct iphdr) + RX_ESP_HDR_SIZE);
	    }
	    else
	    {
                                 ...
                                 int ivsize)
{
    ...
    if((trans_req->dfrags == 1) && (!trans_req->alloc_buff_spu))
    {
	    ...
	    ptr = ptr - (ivsize + sizeof (struct iphdr) + RX_ESP_HDR_SIZE);
	    buffer_len = (nbytes + ivsize + sizeof(struct iphdr) 
                                            + RX_ESP_HDR_SIZE
                                            + RX_MSG_HDR_SIZE 
                                            + RX_STS_SIZE);
	    trans_req->dlen = buffer_len;
	    pkt_frag = kmalloc(sizeof(struct spu_pkt_frag), GFP_ATOMIC);
    ...
    else
    {
	    ...
	    buffer_len = (nbytes + ivsize + sizeof(struct iphdr) 
                                            ...
                                            + RX_STS_SIZE);
	    trans_req->dlen = buffer_len;
	    trans_req->dbuf = kmalloc(buffer_len, GFP_ATOMIC);
        ...
        //buffer_len = buffer_len - 64;
        out_dma_len = trans_req->dlen;
        //out_dma_len = trans_req->dlen - 64;
        ...
        while(nloops)
        {
            ...
            if(out_dma_len >= SPU_CHAIN_SIZE)
                pkt_frag->len = SPU_CHAIN_SIZE;
            //else if(trans_req->dlen <= SPU_CHAIN_SIZE)
            else if(buffer_len < SPU_CHAIN_SIZE)
            //pkt_frag->len = trans_req->dlen;
                pkt_frag->len = out_dma_len;
            else
            //pkt_frag->len = ((out_dma_len/64) * 64);
                pkt_frag->len = out_dma_len;
#endif
                                 ...
                                 int ivsize)
{
    ...
    {
	    trans_req->slen = cryptlen + ivsize + sizeof (struct iphdr) 
                                       + RX_ESP_HDR_SIZE;
    }
    else
    {
	    trans_req->slen = cryptlen + ivsize + sizeof (struct iphdr) 
                                       + RX_ESP_HDR_SIZE + authsize;
    }
