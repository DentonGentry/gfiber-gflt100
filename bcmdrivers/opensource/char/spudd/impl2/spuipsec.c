...
//extern unsigned char bcm_buf_hdr[24];
                               ...
                               CCH * cch)
{
	...
	{
	    bufDesc->offsetMAC = pip->ihl * 4;

	    if (op->flags.dir == BCMSAD_DIR_INBOUND)
	    {
		bufDesc->lengthMAC = len - ((pip->ihl * 4) + SIZE_IPSEC_ICV);
	    }
	    else
	    {
		bufDesc->lengthMAC = len - (pip->ihl * 4);
	    }
	}
	else
	{
    ...
    {
	bufDesc->offsetCrypto = SIZE_ESP_HEADER + devsa->ivLen + (pip->ihl * 4);
    }
    else
    {
	...
	if (op->flags.dir == BCMSAD_DIR_INBOUND)
	{
	    if (op->encrAlg != CRYPTO_ALG_NULL)
	    {
		bufDesc->lengthCrypto = len - ((pip->ihl * 4) + 
	...
	else
	{
		...
		bufDesc->lengthCrypto = len -
		              ((pip->ihl * 4) + SIZE_ESP_HEADER + devsa->ivLen);
	    }
	    else
	    {
		bufDesc->lengthCrypto = 0;
	...
	bufDesc->offsetIV = (pip->ihl * 4) + SIZE_ESP_HEADER;
    }
    else
    {
        bufDesc->offsetICV = (pip->ihl * 4) + sizeof(struct ip_auth_hdr);
    }
...
//    SCTX_INSAU *insau = NULL;
    unsigned char *p;
    unsigned char *pOutHdr = NULL;
    SACTX DEV_SA;
    SACTX *devsa = &DEV_SA;
    int status = 0;
    uint32_t stsWord = 0;
    BDESC bufDesc;
    struct iphdr *pip = (struct iphdr *)trans_req->sfrags_list->buf;
    u_int8_t *spu_cmd_buf = NULL;
    ...
    if ((spu_cmd_buf = kmalloc (BCM_XTRA_DMA_HDR_SIZE, GFP_ATOMIC)) == NULL)
    {
	printk(KERN_ERR "Allocate DMA Context Memory error\n");
	goto bcm_error1;
    }

    trans_req->cmd_buf = (void *)spu_cmd_buf;

    spu_cmd_ptr = (unsigned char *)IPSEC_SPU_ALIGN (spu_cmd_buf, ALIGN_SIZE);

    p = (u_int8_t *) CACHE_TO_NONCACHE ((uint32_t) spu_cmd_ptr);
    spu_cmd_ptr = p;

    devsa->dmaStatus = &stsWord;
    devsa->dmactx = spu_cmd_ptr;

    pOutHdr = (unsigned char *) (&spu_cmd_buf[0] + 
                                        (BCM_XTRA_DMA_HDR_SIZE - OUT_HDR_OFST));
    devsa->outHdr = pOutHdr;
    ...
    status = spu_buff_desc_init (spu_cmd_ptr, &spu_cmd_offset, &bufDesc, 
                                 op, pip, trans_req->slen, devsa, cch);
...
int spu_finish_processing (uint8_t *addr)
{
    ...
    if((pdev_ctrl->test_mode) || (!((uint32)ptr2 & 0x0000FFFF))) 
    {
	return 0;
    }
