...
void bcmPktDma_EthSetPhyRate_Dqm(uint8 port, uint8 enable, int kbps)
{
    ...
    if(enable)
    {
        fapTm_kbpsToTokens(kbps, &fapMsg.tm.tokens, &fapMsg.tm.bucketSize);
    }
    else
    {
        fapMsg.tm.tokens = 0;
        fapMsg.tm.bucketSize = 0;
    }

    printk("FAP TM: %dKbps -> tokens <%lu>, bucketSize <%lu>\n",
           kbps, fapMsg.tm.tokens, fapMsg.tm.bucketSize);

    for(fapIdx=0; fapIdx<NUM_FAPS; ++fapIdx)
    {
        bcmPktDma_xmit2Fap(fapIdx, FAP_MSG_TM_CONFIG, &fapMsg);
    }
#endif
}
