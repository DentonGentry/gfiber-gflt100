...
void br_mcast_handle_netdevice_events(struct net_device *ndev, unsigned long event)
{
    ...
    struct net_br_mld_mc_rep_entry *mld_rep_entry, *mld_rep_entry_n;
#endif
                ...
                if((dev->priv_flags & IFF_EBRIDGE) && (br)) {
                    ...
                    if((!memcmp(ndev->name, dst->wan_name, IFNAMSIZ)) ||
                        ...
                        (!memcmp(ndev->name, dev->name, IFNAMSIZ))) {
                            /* inform mcpd that the snooping entries 
                               are being removed */
                            if ( br->igmp_snooping )
                            {
                    ...
                    if((!memcmp(ndev->name, mld_dst->wan_name, IFNAMSIZ)) ||
                        ...
                        (!memcmp(ndev->name, dev->name, IFNAMSIZ))) {
                            list_for_each_entry_safe(mld_rep_entry, 
                                     mld_rep_entry_n, &mld_dst->rep_list, list) {
                                list_del(&mld_rep_entry->list);
                                kfree(mld_rep_entry);
                            }
